...
export interface PDFEncadrant {
  nom: string;
  prenom: string;
  email: string;
}

export interface PDFDossier {
  id: number;
  titre: string;
  description: string;
  anneeAcademique: string;
  type: 'solo' | 'binome';
  candidatPrincipal: PDFCandidat | null;
  candidatBinome?: PDFCandidat | null;
  encadrant: PDFEncadrant | null;
}

export const generateFicheDepotPDF = (dossier: PDFDossier, targetCandidateEmail?: string) => {
  const doc = new jsPDF();
  ...
  doc.setFont('helvetica', 'normal');
  const dateNaissance = primaryCandidat?.dateNaissance || 'N/A';
  const lieuNaissance = primaryCandidat?.lieuNaissance || 'N/A';
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(primaryCandidat?.telephone || 'N/A', 70, yPos);
  yPos += lineSpacing;
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(primaryCandidat?.classe || 'L3 GL', 70, yPos);
  yPos += lineSpacing + 10;
  ...
  // Sauvegarder le PDF
  const fileName = `Fiche_Depot_${primaryCandidat?.nom || 'Dossier'}_${dossier.id}.pdf`;
  doc.save(fileName);
};

export const generateNotesSuiviPDF = (dossier: any, notes: any[]) => {
  const doc = new jsPDF();
  const pageWidth = doc.internal.pageSize.getWidth();
