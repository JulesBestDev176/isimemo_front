...
const Partners = () => {
    return (_jsxs("section", { className: "section bg-gray-50", children: [_jsxs("div", { className: "text-center max-w-3xl mx-auto mb-16", children: [_jsx(motion.h2, { className: "text-3xl font-bold mb-4 text-navy", initial: { opacity: 0, y: 20 }, whileInView: { opacity: 1, y: 0 }, viewport: { once: true }, transition: { duration: 0.6 }, children: "Nos Partenaires" }), _jsx(motion.p, { className: "text-lg text-navy-700", initial: { opacity: 0, y: 20 }, whileInView: { opacity: 1, y: 0 }, viewport: { once: true }, transition: { duration: 0.6, delay: 0.2 }, children: "Nous collaborons avec les leaders mondiaux du secteur technologique." })] }), _jsx(motion.div, { className: "grid grid-cols-2 md:grid-cols-4 gap-8", variants: container, initial: "hidden", whileInView: "show", viewport: { once: true }, children: partners.map((partner, index) => (_jsx(motion.div, { className: "bg-white p-8 rounded-xl shadow-sm flex items-center justify-center hover:shadow-md transition-shadow", variants: item, whileHover: { y: -5 }, children: _jsx("img", { src: partner.image, alt: partner.name, className: "max-h-12 w-auto object-contain grayscale hover:grayscale-0 transition-all" }) }, index))) })] }));
};
