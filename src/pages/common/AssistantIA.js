...
const AssistantIA = () => {
            ...
            messages: [
                {
                    id: "welcome",
                    role: "assistant",
                    content: "Bonjour ! Je suis votre assistant IA. Je peux vous aider avec vos questions sur la méthodologie de recherche, la structuration de votre mémoire, les sources et références, ou tout autre sujet lié à votre travail académique. Comment puis-je vous aider aujourd'hui ?",
                    timestamp: new Date(),
                },
            ],
    ...
    const handleSendMessage = () => __awaiter(void 0, void 0, void 0, function* () {
        if (!inputMessage.trim() || isLoading || !currentConversationId)
            return;
        const userMessage = {
            id: `user-${Date.now()}`,
            role: "user",
        ...
        setConversations((prev) => prev.map((conv) => {
            if (conv.id === currentConversationId) {
                ...
                const newTitle = conv.title === "Nouvelle discussion" && conv.messages.length === 1
                    ? userMessage.content.substring(0, 20) +
                        (userMessage.content.length > 20 ? "..." : "")
                    : conv.title;
                return Object.assign(Object.assign({}, conv), { messages: updatedMessages, title: newTitle, lastMessageDate: new Date() });
            }
            return conv;
        }));
        setInputMessage("");
        ...
        setTimeout(() => {
            ...
            setConversations((prev) => prev.map((conv) => {
                if (conv.id === currentConversationId) {
                    return Object.assign(Object.assign({}, conv), { messages: [...conv.messages, assistantMessage], lastMessageDate: new Date() });
                }
                return conv;
            }));
            setIsLoading(false);
...
Pourriez-vous reformuler votre question en rapport avec l'un de ces sujets ?`;
    ...
    return (_jsxs("div", { className: "bg-gray-50 flex", style: { height: "80vh" }, children: [_jsxs("div", { className: "w-80 bg-white border-r border-gray-200 flex flex-col", children: [_jsxs("div", { className: "p-4 border-b border-gray-200", children: [_jsx("div", { className: "flex items-center justify-between mb-4", children: _jsxs("div", { className: "flex items-center gap-2", children: [_jsx("div", { className: "bg-primary-100 p-2 rounded-lg", children: _jsx(Bot, { className: "h-5 w-5 text-primary-700" }) }), _jsx("h2", { className: "text-lg font-bold text-gray-900", children: "Assistant IA" })] }) }), _jsxs("button", { onClick: handleNewConversation, className: "w-full btn-primary flex items-center justify-center gap-2", children: [_jsx(Plus, { className: "h-4 w-4" }), "Nouvelle discussion"] })] }), _jsx("div", { className: "p-4 border-b border-gray-200", children: _jsxs("div", { className: "relative", children: [_jsx(Search, { className: "absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400" }), _jsx("input", { type: "text", placeholder: "Rechercher...", value: searchQuery, onChange: (e) => setSearchQuery(e.target.value), className: "w-full pl-10 pr-4 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent" })] }) }), _jsx("div", { className: "flex-1 overflow-y-auto", children: filteredConversations.length > 0 ? (_jsx("div", { className: "divide-y divide-gray-200", children: filteredConversations.map((conversation) => {
                                const isActive = conversation.id === currentConversationId;
                                const lastMessage = conversation.messages[conversation.messages.length - 1];
                                return (_jsx("div", { onClick: () => setCurrentConversationId(conversation.id), className: `p-4 cursor-pointer transition-colors ${isActive ? "bg-primary-50 border-l-4 border-primary" : "hover:bg-gray-50"}`, children: _jsxs("div", { className: "flex items-start justify-between", children: [_jsxs("div", { className: "flex-1 min-w-0", children: [_jsx("h3", { className: `font-medium text-sm mb-1 truncate ${isActive ? "text-primary" : "text-gray-900"}`, children: conversation.title.length > 20
                                                            ? conversation.title.substring(0, 20) + "..."
                                                            : conversation.title }), lastMessage && (_jsxs("p", { className: "text-xs text-gray-500 truncate mb-1", children: [lastMessage.content.substring(0, 60), lastMessage.content.length > 60 ? "..." : ""] })), _jsxs("div", { className: "flex items-center gap-2 text-xs text-gray-400", children: [_jsx(Clock, { className: "h-3 w-3" }), _jsx("span", { children: formatDate(conversation.lastMessageDate) })] })] }), _jsx("button", { onClick: (e) => {
                                                    e.stopPropagation();
                                                    handleDeleteConversation(conversation.id);
                                                }, className: "p-1 text-gray-400 hover:text-primary-600 rounded hover:bg-primary-50 ml-2", title: "Supprimer", children: _jsx(Trash2, { className: "h-4 w-4" }) })] }) }, conversation.id));
                            }) })) : (_jsxs("div", { className: "p-8 text-center", children: [_jsx(Bot, { className: "h-12 w-12 text-gray-400 mx-auto mb-3" }), _jsx("p", { className: "text-sm text-gray-600 mb-2", children: "Aucune discussion" }), _jsx("p", { className: "text-xs text-gray-500", children: searchQuery
                                        ? "Aucune discussion trouvée"
                                        : "Créez une nouvelle discussion pour commencer" })] })) })] }), _jsx("div", { className: "flex-1 flex flex-col", children: currentConversation ? (_jsxs(_Fragment, { children: [_jsx("div", { className: "bg-white border-b border-gray-200 px-6 py-4 flex justify-center", children: _jsx("div", { className: "w-full max-w-4xl", children: _jsx("h1", { className: "text-xl font-bold text-gray-900 truncate", children: currentConversation.title.length > 20
                                        ? currentConversation.title.substring(0, 20) + "..."
                                        : currentConversation.title }) }) }), _jsx("div", { className: "flex-1 overflow-y-auto px-6 py-6 bg-white flex justify-center", children: _jsxs("div", { className: "w-full max-w-4xl space-y-6", children: [currentConversation.messages.map((message, index) => (_jsxs(motion.div, { initial: { opacity: 0, y: 10 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.3, delay: index * 0.05 }, className: `flex gap-4 ${message.role === "user" ? "justify-end" : "justify-start"}`, children: [message.role === "assistant" && (_jsx("div", { className: "flex-shrink-0", children: _jsx("div", { className: "bg-primary-100 p-2 rounded-lg", children: _jsx(Bot, { className: "h-5 w-5 text-primary-700" }) }) })), _jsxs("div", { className: `max-w-full sm:max-w-xl lg:max-w-3xl ${message.role === "user" ? "order-2" : ""}`, children: [_jsx("div", { className: `rounded-lg px-4 py-3 ${message.role === "user"
                                                            ? "bg-primary text-white"
                                                            : "bg-gray-100 text-gray-900"}`, children: _jsx("p", { className: "text-sm whitespace-pre-wrap break-words", children: message.content }) }), _jsx("p", { className: "text-xs text-gray-500 mt-1 px-1", children: message.timestamp.toLocaleTimeString("fr-FR", {
                                                            hour: "2-digit",
                                                            minute: "2-digit",
                                                        }) })] }), message.role === "user" && (_jsx("div", { className: "flex-shrink-0 order-3", children: _jsx("div", { className: "bg-gray-200 p-2 rounded-lg", children: _jsx(User, { className: "h-5 w-5 text-gray-600" }) }) }))] }, message.id))), isLoading && (_jsxs(motion.div, { initial: { opacity: 0, y: 10 }, animate: { opacity: 1, y: 0 }, className: "flex gap-4 justify-start", children: [_jsx("div", { className: "flex-shrink-0", children: _jsx("div", { className: "bg-primary-100 p-2 rounded-lg", children: _jsx(Bot, { className: "h-5 w-5 text-primary-700" }) }) }), _jsx("div", { className: "bg-gray-100 rounded-lg px-4 py-3", children: _jsxs("div", { className: "flex items-center gap-2", children: [_jsx(Loader2, { className: "h-4 w-4 text-gray-500 animate-spin" }), _jsx("span", { className: "text-sm text-gray-600", children: "L'assistant r\u00E9fl\u00E9chit..." })] }) })] })), _jsx("div", { ref: messagesEndRef })] }) }), _jsxs("div", { className: "bg-white border-t border-gray-200 px-6 py-4", children: [_jsxs("div", { className: "relative max-w-4xl mx-auto", children: [_jsx("textarea", { ref: inputRef, value: inputMessage, onChange: (e) => setInputMessage(e.target.value), onKeyPress: handleKeyPress, placeholder: "Tapez votre message... (Entr\u00E9e pour envoyer)", className: "w-full px-4 py-2.5 pr-12 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent resize-none overflow-y-auto", rows: 1, style: {
                                                minHeight: "40px",
                                                maxHeight: "120px",
                                                wordWrap: "break-word",
                                                overflowWrap: "break-word",
                                            }, disabled: isLoading, onInput: (e) => {
                                                const target = e.target;
                                                target.style.height = "auto";
                                                const newHeight = Math.min(target.scrollHeight, 120);
                                                target.style.height = `${newHeight}px`;
                                            } }), _jsx("button", { onClick: handleSendMessage, disabled: !inputMessage.trim() || isLoading, className: `absolute right-2 bottom-2 flex items-center justify-center rounded-lg transition-colors ${inputMessage.trim() && !isLoading
                                                ? "bg-primary text-white hover:bg-primary-700"
                                                : "bg-gray-200 text-gray-400 cursor-not-allowed"}`, style: {
                                                width: "36px",
                                                height: "36px",
                                            }, children: isLoading ? (_jsx(Loader2, { className: "h-4 w-4 animate-spin" })) : (_jsx(Send, { className: "h-4 w-4" })) })] }), _jsx("p", { className: "text-xs text-gray-500 mt-2", children: "L'assistant IA peut vous aider avec la m\u00E9thodologie, la structuration de votre m\u00E9moire, les sources et r\u00E9f\u00E9rences." })] })] })) : (_jsx("div", { className: "flex-1 flex items-center justify-center bg-white", children: _jsxs("div", { className: "text-center", children: [_jsx(Bot, { className: "h-16 w-16 text-gray-400 mx-auto mb-4" }), _jsx("h2", { className: "text-xl font-bold text-gray-900 mb-2", children: "Bienvenue sur l'Assistant IA" }), _jsx("p", { className: "text-gray-600 mb-6", children: "Cr\u00E9ez une nouvelle discussion pour commencer \u00E0 poser vos questions" }), _jsxs("button", { onClick: handleNewConversation, className: "btn-primary flex items-center gap-2 mx-auto", children: [_jsx(Plus, { className: "h-4 w-4" }), "Nouvelle discussion"] })] }) })) })] }));
};
