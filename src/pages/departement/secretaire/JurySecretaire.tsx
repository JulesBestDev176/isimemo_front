...
// Types locaux si pas de fichier types
...
const JuryListTab: React.FC<{
  jurys: Jury[];
  onViewDetails: (jury: Jury) => void;
  onEditJury: (jury: Jury) => void;
    ...
    <div>
      <div className="bg-white border border-gray-200 p-6 mb-6">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          >
            Générer un jury automatiquement
          </SimpleButton>
          <SimpleButton 
            variant="primary" 
...
const JuryChef: React.FC = () => {
  ...
  const [juryToEdit, setJuryToEdit] = useState<Jury | null>(null);

  const { user } = useAuth();

  const openJuryDetails = (jury: Jury) => {
  ...
  const handleGenerateJurys = (nouveauxJurys: any[]) => {
    const jurysAvecId = nouveauxJurys.map((jury, index) => ({
      ...jury,
  ...
  const handleSaveJury = (jury: Jury) => {
...
const JuryDetailsModal: React.FC<{
  ...
  const getRoleIcon = (role: string) => {
    switch (role) {
      case 'president': return <Gavel className="h-4 w-4" />;
      case 'rapporteur': return <FileText className="h-4 w-4" />;
      case 'examinateur': return <UserCheck className="h-4 w-4" />;
      ...
      >
              ...
              {jury.membres.map((membre, index) => (
                <div key={index} className="border border-gray-200 p-3">
                  ...
                  <p className="text-sm text-gray-600 flex items-center">
                    ...
                    {membre.professeur.institution} - {membre.professeur.specialite}
                  </p>
                </div>
              ))}
            </div>
        ...
        <div className="mt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {jury.etudiants.map((etudiant, index) => {
              ...
              return (
                <div key={index} className="border border-gray-200 p-3">
                  ...
                  <div className="flex items-center gap-2 mt-2 flex-wrap">
                    <Badge variant="default">{etudiant.niveau}</Badge>
                    <Badge variant="outline">{etudiant.specialite}</Badge>
                    <Badge variant="outline">Session {getSessionLabel(etudiant.niveau)}</Badge>
                  </div>
                  {encadrant && (
                  ...
                  {etudiant.memoire && (
                    <div className="mt-2">
                      <p className="text-sm font-medium text-gray-700">{etudiant.memoire.titre}</p>
                      <Badge 
                        variant={
                          etudiant.memoire.etat === 'valide' ? 'secondary' :
                          etudiant.memoire.etat === 'soumis' ? 'destructive' : 'outline'
                        }
                        className="mt-1"
                      >
                        {etudiant.memoire.etat}
                      </Badge>
                    </div>
                  )}
                </div>
...
const AutoGenerateJuryModal: React.FC<{
...
}> = ({ isOpen, onClose, onGenerate, professeurs, etudiants, userDepartment }) => {
  const [anneeAcademique, setAnneeAcademique] = useState('2024-2025');
  const [selectedNiveau, setSelectedNiveau] = useState<'licence' | 'master'>('master');
  const [selectedSession, setSelectedSession] = useState<'septembre' | 'decembre' | 'speciale'>('septembre');
  ...
  const etudiantsDepart = etudiants.filter(e => 
    e.department === userDepartment && 
      ...
      >
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Créer un jury pour le département {userDepartment}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setSelectedNiveau(e.target.value as 'licence' | 'master')}
                className="w-full px-3 py-2 border border-gray-300 focus:outline-none focus:border-navy focus:ring-1 focus:ring-navy"
              >
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setSelectedSession(e.target.value as 'septembre' | 'decembre' | 'speciale')}
                className="w-full px-3 py-2 border border-gray-300 focus:outline-none focus:border-navy focus:ring-1 focus:ring-navy"
              >
                <option value="septembre">Septembre</option>
                <option value="decembre">Décembre</option>
          ...
          <div className="bg-blue-50 border border-blue-200 p-4">
            <h4 className="font-medium text-blue-900 mb-2">Aperçu de la génération :</h4>
            <div className="text-sm text-blue-800 space-y-2">
              <p><strong>Département :</strong> {userDepartment}</p>
              <p><strong>Professeurs disponibles :</strong> {professeursDepart.length}</p>
              <p><strong>Étudiants éligibles :</strong> {etudiantsDepart.length} ({selectedNiveau} - session {getSessionLabel(selectedSession)})</p>
              <p><strong>Jury généré :</strong> 1 jury avec {etudiantsDepart.length} étudiants</p>
            </div>
          </div>

          <div className="bg-orange-50 border border-orange-200 p-4">
            <h4 className="font-medium text-orange-900 mb-2">Règles de génération :</h4>
            <ul className="text-sm text-orange-800 space-y-1">
              <li>• Seuls les professeurs et étudiants du département {userDepartment} sont inclus</li>
              <li>• Tous les étudiants du niveau et session sélectionnés seront dans le même jury</li>
              <li>• <strong>Master :</strong> Le professeur le plus gradé sera président</li>
              <li>• <strong>Licence :</strong> Le président est choisi aléatoirement</li>
              <li>• L'encadrant d'un étudiant ne peut pas être membre de son jury</li>
              <li>• Le jury aura exactement 3 membres (président, rapporteur, examinateur)</li>
            </ul>
          </div>

          {etudiantsDepart.length > 0 && (
            <div>
              <h4 className="font-medium text-gray-900 mb-3">
                Étudiants qui seront inclus ({etudiantsDepart.length})
              </h4>
              <div className="max-h-48 overflow-y-auto border border-gray-200">
                <div className="space-y-2 p-4">
                  {etudiantsDepart.map((etudiant, index) => {
                      ...
                      <div key={index} className="flex items-center justify-between p-2 bg-gray-50">
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            {etudiant.niveau} - {etudiant.classe}
                          </p>
                        </div>
                        <div className="text-right">
                          <Badge variant="outline">
                            {getSessionLabel(etudiant.niveau)}
                          </Badge>
                          {encadrant && (
                            <p className="text-xs text-gray-500 mt-1">
                              Encadrant: {encadrant.prenom} {encadrant.nom}
          ...
          {professeursDepart.length > 0 && (
            <div>
              <h4 className="font-medium text-gray-900 mb-3">
                Professeurs disponibles ({professeursDepart.length})
              </h4>
              <div className="max-h-48 overflow-y-auto border border-gray-200">
                <div className="space-y-2 p-4">
                  {professeursDepart.map((prof, index) => (
                    <div key={index} className="flex items-center justify-between p-2 bg-gray-50">
                      <div>
                        <p className="font-medium text-gray-900">
                        ...
                        <p className="text-sm text-gray-600">
                          {prof.grade} - {prof.institution}
                        </p>
                      </div>
                      <Badge variant="secondary">
                        Niveau {prof.gradeNiveau}
                      </Badge>
                    </div>
                  ))}
                </div>
        ...
        <div className="flex justify-end space-x-4 mt-8 pt-6 border-t border-gray-200">
          ...
          <SimpleButton 
            ...
            disabled={etudiantsDepart.length === 0 || professeursDepart.length < 3}
            icon={<Shuffle className="h-4 w-4" />}
          >
            Générer le jury
          </SimpleButton>
...
const EditJuryModal: React.FC<{
  ...
  onClose: () => void;
  jury: Jury | null;
  onSave: (jury: Jury) => void;
}> = ({ isOpen, onClose, jury, onSave }) => {
  const [form, setForm] = useState<Jury | null>(jury);
