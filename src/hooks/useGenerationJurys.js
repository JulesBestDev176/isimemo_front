...
import { mockDossiers, StatutDossierMemoire, EtapeDossier } from '../models/dossier/DossierMemoire';
...
export const useGenerationJurys = () => {
     ...
     */
    const genererPropositions = useCallback((annee, session, niveau, departement, sessionDateDebut, sessionDateFin) => __awaiter(void 0, void 0, void 0, function* () {
        setIsGenerating(true);
        setError(null);
        try {
            // 1. Récupération des données
            const dossiers = mockDossiers.filter(d => d.statut === StatutDossierMemoire.VALIDE &&
                d.etape === EtapeDossier.SOUTENANCE &&
                d.anneeAcademique === annee);
            const professeurs = mockProfesseurs.filter(p => {
                var _a;
                return p.estDisponible &&
                    (p.departement === departement || ((_a = p.departement) === null || _a === void 0 ? void 0 : _a.includes(departement)));
            });
            const presidentsEligibles = mockAttributions
                .filter(a => a.typeRole === TypeRole.PRESIDENT_JURY_POSSIBLE &&
    ...
    function trouverJuryValide(dossiers, tousProfesseurs, presidentsEligibles) {
        const encadrantsIds = new Set(dossiers.map(d => { var _a; return (_a = d.encadrant) === null || _a === void 0 ? void 0 : _a.idProfesseur; }).filter(id => id !== undefined));
        const candidatsPresident = presidentsEligibles.filter(p => !encadrantsIds.has(p.idProfesseur) &&
            tousProfesseurs.some(tp => tp.idProfesseur === p.idProfesseur));
        if (candidatsPresident.length === 0)
            return null;
        const president = candidatsPresident[Math.floor(Math.random() * candidatsPresident.length)];
        const autresProfesseurs = tousProfesseurs.filter(p => p.idProfesseur !== president.idProfesseur &&
            !encadrantsIds.has(p.idProfesseur));
        if (autresProfesseurs.length < 2)
            return null;
...
// Utilitaires
function generateDates(debut, fin, count) {
    const dates = [];
