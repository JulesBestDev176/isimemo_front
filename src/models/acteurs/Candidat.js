...
export const getCandidatsByMemoireId = (memoireId) => {
    return mockCandidats.filter(c => c.memoireId === memoireId);
};
/**
 * Récupère l'ID du candidat basé sur l'email de l'utilisateur
 */
export const getCandidatIdByEmail = (email) => {
    const candidat = getCandidatByEmail(email);
    return candidat === null || candidat === void 0 ? void 0 : candidat.idCandidat;
};
/**
 * Vérifie les identifiants d'un candidat (pour la connexion)
 */
export const authenticateCandidat = (email, password) => {
    const candidat = mockCandidats.find(c => c.email.toLowerCase() === email.toLowerCase() && c.motDePasse === password);
