...
const createRessourceSauvegardee = (idSauvegarde, idRessource, idEtudiant, idProfesseur, dateSauvegarde) => {
...
export const mockRessourcesSauvegardees = ressourcesSauvegardeesData
    .map(data => createRessourceSauvegardee(data.idSauvegarde, data.idRessource, data.idEtudiant, data.idProfesseur, data.dateSauvegarde))
    .filter((rs) => rs !== null);
// ============================================================================
// HELPER FUNCTIONS
// ============================================================================
export const getRessourcesSauvegardees = (idEtudiant) => {
    return mockRessourcesSauvegardees.filter(rs => rs.idEtudiant === idEtudiant);
};
export const getRessourcesSauvegardeesProfesseur = (idProfesseur) => {
    return mockRessourcesSauvegardees.filter(rs => rs.idProfesseur === idProfesseur);
};
export const getRessourceSauvegardeeById = (id) => {
...
export const addRessourceSauvegardee = (idRessource, idEtudiant, idProfesseur) => {
    // Vérifier si la ressource existe déjà
    const existing = mockRessourcesSauvegardees.find(rs => rs.idRessource === idRessource &&
...
export const removeRessourceSauvegardee = (idRessource, idEtudiant, idProfesseur) => {
    const index = mockRessourcesSauvegardees.findIndex(rs => rs.idRessource === idRessource &&
        ((idEtudiant && rs.idEtudiant === idEtudiant) || (idProfesseur && rs.idProfesseur === idProfesseur)));
    if (index !== -1) {
...
export const isRessourceSauvegardee = (idRessource, idEtudiant, idProfesseur) => {
    return mockRessourcesSauvegardees.some(rs => rs.idRessource === idRessource &&
        ((idEtudiant && rs.idEtudiant === idEtudiant) || (idProfesseur && rs.idProfesseur === idProfesseur)));
};
