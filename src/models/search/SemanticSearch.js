...
export const highlightMatches = (text, keywords) => {
    ...
    sentences.forEach(sentence => {
        ...
        const hasMatch = keywords.some(keyword => sentenceLower.includes(keyword.toLowerCase()));
...
export class SemanticSearchEngine {
    static calculateRelevance(query, resource) {
        ...
        const commonKeywords = queryKeywords.filter(qk => resourceKeywords.some(rk => rk.includes(qk) || qk.includes(rk)));
    ...
    static applyFilters(resource, filters) {
        if (!filters)
            return true;
        ...
        if (year) {
            if (filters.anneeMin && year < filters.anneeMin)
                return false;
            if (filters.anneeMax && year > filters.anneeMax)
                return false;
        }
        if (filters.auteurs && filters.auteurs.length > 0) {
            const hasAuthor = filters.auteurs.some(filterAuthor => resource.auteur.toLowerCase().includes(filterAuthor.toLowerCase()));
            if (!hasAuthor)
                return false;
        }
        return true;
    }
    static search(searchQuery, resources) {
        var _a;
        ...
        const results = resources
            ...
            const highlights = highlightMatches(resource.description || resource.titre, keywords);
            let explanation = '';
            if (score > 70) {
        ...
        else if (sortBy === 'date' && ((_a = resources[0]) === null || _a === void 0 ? void 0 : _a.datePublication)) {
            results.sort((a, b) => {
    ...
    static getSuggestions(partialQuery, recentSearches) {
